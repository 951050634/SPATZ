...
// LMUL=4, SEW=16
// Optimization: Explicit `flh` + Dual Pointers
// ---------------
                 ...
                 const unsigned int p_start, const unsigned int p_end)
{
    ...
    while (p < p_end)
    {
        ...
        for (unsigned int m = m_start; m < m_end; m += 2)
        {
            ...
            // Explicit FLH using pointers
            // Offset 0 = Real, Offset 2 = Imag (2 bytes for _Float16)
            ...
            while (n < N)
            {
                ...
                // Load Next Scalars (Explicit FLH + Pointer Inc)
                asm volatile("flh %[t], 0(%[a])" : [t] "=f"(ar0) : [a] "r"(p_a0));
                asm volatile("flh %[t], 2(%[a])" : [t] "=f"(ai0) : [a] "r"(p_a0));
                p_a0++;
                asm volatile("flh %[t], 0(%[a])" : [t] "=f"(ar1) : [a] "r"(p_a1));
...
// LMUL=2, SEW=16
// Optimization: Explicit `flh` + Quad Pointers
// ---------------
    ...
    while (p < p_end)
    {
        ...
        for (unsigned int m = m_start; m < m_end; m += 4)
        {
            ...
            while (n < N)
            {
                ...
                // Load 4 rows of scalars explicitly
                // Row 0
            ...
            chalf *c__ = c_ + m * P; // cfloat* needed to match pointer arithmetic? No, use chalf*
            ...
            asm volatile("vsseg2e16.v v0, (%0);" ::"r"(c_ptr));
            c_ptr += P;
            ...
            asm volatile("vsseg2e16.v v4, (%0);" ::"r"(c_ptr));
            c_ptr += P;
            ...
            asm volatile("vsseg2e16.v v8, (%0);" ::"r"(c_ptr));
            c_ptr += P;
            ...
            asm volatile("vsseg2e16.v v12, (%0);" ::"r"(c_ptr));
        }
        p += gvl;
    }
}