#!/usr/bin/env python3
# Generate data for complex matmul (dp-cmatmul). Copy of dp-cdotp script; outputs to ../data.
...
def get_c_type_name(prec):
    ...
    if prec == 16: return "chalf", "_Float16"
...
def emit_cmatmul(name, **kwargs):
    ...
    c_struct, _ = get_c_type_name(prec)
    s = '#include "layer.h"\n\n'
    s += f"// Kernel: CMATMUL (C += A * B), Precision: {prec}-bit. A MxK, B KxN, C MxN.\n"
...
def main():
    ...
    emit_str = "// Generated by gen_data.py\n" + emit_cmatmul("cmatmul", M=M, N=N, K=K, prec=prec, A=A, B=B, C=C, result=result)
