...
static inline int cp_check(const cbyte calc, const cbyte ref) {
  int dr = (int)calc.real - (int)ref.real;
  int di = (int)calc.imag - (int)ref.imag;
  if (dr < 0) dr = -dr;
  if (di < 0) di = -di;
  return (dr > 1) || (di > 1);
}
...
int main() {
  ...
  if (cid == 0) {
    a = (cbyte *)snrt_l1alloc(M * N * sizeof(cbyte));
    b = (cbyte *)snrt_l1alloc(N * P * sizeof(cbyte));
    c = (cbyte *)snrt_l1alloc(M * P * sizeof(cbyte));
  }
  snrt_cluster_hw_barrier();

  if (cid == 0) {
    snrt_dma_start_1d(a, cmatmul_a, M * N * sizeof(cbyte));
    snrt_dma_start_1d(b, cmatmul_b, N * P * sizeof(cbyte));
    snrt_dma_start_1d(c, cmatmul_c, M * P * sizeof(cbyte));
    snrt_dma_wait_all();
  }
  snrt_cluster_hw_barrier();
